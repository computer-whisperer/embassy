...
impl<'d, T: Instance> I2c<'d, T, Async> {
    ...
    async fn read_async_internal(&mut self, buffer: &mut [u8], restart: bool, send_stop: bool) -> Result<(), Error> {
        ...
        // Clear any stale STOP-detect so the read wait below only observes a
        // STOP generated by this read's own transfer.
            ...
            let res = self
                .wait_on(
                    |me| {
                        ...
                        } else if T::regs().ic_raw_intr_stat().read().stop_det() {
                            // The transfer has ended (STOP seen on the bus)
                            // but fewer bytes than this batch expected are in
                            // the RX FIFO. RX_FULL can never assert now, so
                            // fail fast instead of blocking until the caller
                            // gives up. (Observed on hardware: the final byte
                            // of a transfer occasionally never commits to the
                            // RX FIFO even though it was ACKed on the wire.)
                            Poll::Ready(Err(Error::Abort(AbortReason::Other(rxfifo as u32))))
                        } else {
                            Poll::Pending
                        }
                    },
                    |_me| {
                        ...
                        p.ic_intr_mask().modify(|w| {
                            ...
                            // Also wake on STOP so a short/ended transfer
                            // can't hang.
